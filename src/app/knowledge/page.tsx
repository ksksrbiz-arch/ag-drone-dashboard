  ...
  async function onFiles(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      ...
      if (!TEXT_EXT.includes(ext)) {
        setError(`"${file.name}" isn't a text file. Supported: ${TEXT_EXT.join(', ')}. Paste its contents as a note instead.`)
        continue
      }
    ...
    <div className="p-4 sm:p-8 max-w-5xl mx-auto">
      <header className="mb-6">
        ...
        <p className="text-slate-500 text-sm mt-1">
          Files, folders & notes the Sidekick assistant uses as context — pricing sheets, SOPs, call scripts, agronomy references. Ask Sidekick a reference question and it searches here.
        </p>
      </header>
          ...
          <form onSubmit={addNote} className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <input
                ...
                className="hidden"
              />
              <span className="text-xs text-slate-400">Uploads go to the “{folder.trim() || 'General'}” folder · text files up to {fmtBytes(MAX_BYTES)}</span>
            </div>
          </form>
        </div>
      )}
