    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-card p-5 space-y-5">
      <div>
        <div className="flex items-start justify-between gap-2">
          ...
          {isStaff ? (
            ...
            <span className={`text-xs font-medium rounded-full px-2 py-1 ${STATUS_META[customer.status]}`}>{customer.status}</span>
          )}
        </div>
      ...
      <div>
        ...
        {isStaff ? (
          ...
          <p className="text-sm text-slate-600 whitespace-pre-wrap">{notes || <span className="text-slate-400">No notes.</span>}</p>
        )}
      </div>
