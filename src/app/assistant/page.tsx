  ...
  return (
    <div className="p-6 md:p-8 max-w-3xl mx-auto animate-fade flex flex-col h-full min-h-[70vh]">
      ...
      <form onSubmit={e => { e.preventDefault(); send(input) }} className="mt-3 flex items-center gap-1.5 shrink-0">
        ...
        <input value={input} onChange={e => setInput(e.target.value)} placeholder={listening ? 'Listening…' : 'Ask the assistant…'} className="tap flex-1 min-w-0 text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500" />
        {sending ? (
